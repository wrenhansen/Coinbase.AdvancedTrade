    ...
    public sealed class WebSocketManager : IDisposable
    {
        ...
        private async ValueTask ReceiveMessagesAsync()
        {
            ...
            while (IsWebSocketOpen)
            {
                ...
                // Copy the received message segment to avoid buffer reuse issues.
                var segmentCopy = new byte[result.Count];
                Buffer.BlockCopy(buffer, 0, segmentCopy, 0, result.Count);
                messageSegments.Add(new ArraySegment<byte>(segmentCopy, 0, result.Count));
